...
function songStage() {
  ...
  if (room.phase === 'stealing') {
    ...
    return `<section class="song-stage steal-stage"><div><span class="eyebrow">PLACEMENT LOCKED · ANSWER STILL HIDDEN</span><h2>A chance to steal.</h2><p>${esc(active.name)} has locked in a gap. Opponents can spend one token to pick another gap on this timeline.</p><p>The first correct steal wins if the playing team is wrong. Same-year ties go to the playing team. Spent tokens are always lost.</p><span class="steal-clock" role="timer">Steal window: <strong id="steal-countdown"></strong></span></div><div class="steal-status"><span class="token-balance">◉ ${tokenLabel(own?.tokens || 0)}</span><p>${myTurn() ? 'Your answer is locked. Waiting for opponents to steal or pass.' : decided ? 'Your team’s decision is locked. Waiting for the other opponents.' : canSteal() ? 'Pick a different gap below and spend one token, or pass for free.' : 'You need a token to steal. Listen along for the reveal.'}</p>${canSteal() ? '<button class="button button-outline" data-action="pass">Pass · keep your token</button>' : ''}</div></section>`;
  }
  if (room.phase === 'reveal') {
    ...
    return `<section class="song-stage reveal-stage ${result.correct || stolen ? 'reveal-correct' : 'reveal-miss'}"><div class="reveal-copy"><span class="eyebrow">${result.skipped ? 'A LITTLE SKIP IN THE RECORD' : stolen ? 'A WELL-PLACED TOKEN' : result.correct ? 'RIGHT ON TIME' : 'A GOOD SONG. A DIFFERENT YEAR.'}</span><h2>${result.skipped ? 'On to the next one.' : stolen ? `${esc(recipient.name)} stole it.` : result.correct ? myTurn() ? 'You know your stuff.' : `${esc(active.name)} nailed it.` : 'Almost a classic.'}</h2><h3>${esc(result.card.title)}</h3><p>${esc(result.card.artist)}</p><span class="result-note">${result.skipped ? 'The host ended this turn. No cards or tokens awarded.' : stolen ? `The card is now in ${esc(recipient.name)}’s timeline.` : result.correct ? 'One more memory on the timeline.' : 'No card this time. There’s always the next song.'}</span>${!result.skipped ? `<p class="token-result ${result.tokenEarned ? 'token-earned' : ''}">${result.tokenEarned ? `◉ +1 token for ${esc(active.name)}: artist and title matched, regardless of placement.` : 'No token earned: both the artist and title must match at least 80%.'}</p>` : ''}${round.steals.length ? `<p class="result-note">${round.steals.length} ${round.steals.length === 1 ? 'steal token was' : 'steal tokens were'} spent.</p>` : ''}<button class="button button-dark" data-action="next" ${!isHost() && !myTurn() ? 'disabled' : ''}>${room.winners?.length ? 'See the results' : 'Next turn'}${icon('arrow')}</button></div><div class="reveal-year"><span>RELEASED IN</span><strong>${result.card.year}</strong><small>${room.library === 'demo' ? 'FICTIONAL DEMO YEAR' : 'ANOTHER MOMENT IN MUSIC'}</small><div class="year-ring"></div></div></section>`;
  }
  return `<section class="song-stage"><div class="song-record">${vinyl('playing-vinyl', '?')}<div class="record-shadow"></div></div><div class="song-copy"><span class="eyebrow">${myTurn() ? 'YOUR EARS. YOUR CALL.' : `${esc(active.name.toUpperCase())} IS ON THE DECKS`}</span><h2>Where does<br>this one <em>belong?</em></h2><p>${myTurn() ? 'Listen, take a guess, and find its place in your timeline.' : `Listen along while ${esc(active.name)} finds a place for this song.`}</p><div class="audio-status"><span class="equalizer"><i></i><i></i><i></i><i></i></span><span id="playback-label">Loading the clip…</span><span id="playback-time">0:00</span></div><div class="audio-progress"><div id="audio-progress-fill"></div></div><div class="audio-actions"><button class="button button-small button-outline" data-action="replay" ${!isHost() && !myTurn() ? 'disabled' : ''}>${icon('replay')}Replay for everyone</button><button class="text-button" data-action="retry-audio" id="retry-audio" hidden>Retry audio</button></div><p id="audio-error" class="form-error" role="status"></p>${myTurn() ? `<div class="token-actions"><span class="token-balance">◉ ${tokenLabel(active.tokens)}</span><button class="button button-small button-outline" data-action="skip" ${active.tokens < 2 || !room.remaining || !state.connected ? 'disabled' : ''}>${icon('skip')}New song · 2 tokens</button><small>${room.remaining ? 'Keep your turn. Spend two tokens for a replacement song.' : 'No replacement songs left.'}</small></div>` : ''}${isHost() ? '<button class="text-button host-discard" data-action="discard" title="Use for broken clips or disconnected players. This gives up the turn without awarding a card or token.">Host: end turn without a guess</button>' : ''}</div></section>`;
}

function recognitionForm() {
  return `<form id="recognition-form" class="recognition-form"><div class="recognition-heading"><h3>Know this song? Earn a token.</h3><p>Get both the artist and title at least 80% right. Spaces and capitalization do not count. One featured artist is enough. Earn the token even if your placement is wrong.</p></div><div class="form-grid"><label class="field">ARTIST<input id="guess-artist" maxlength="100" autocomplete="off" placeholder="One credited artist" value="${esc(state.artistGuess)}"></label><label class="field">SONG TITLE<input id="guess-title" maxlength="100" autocomplete="off" placeholder="What’s this song called?" value="${esc(state.titleGuess)}"></label></div><p class="small muted">Optional · Submitted with your placement. One guess per team, per song.</p></form>`;
}
...
function timeline() {
  ...
  const gap = n => {
    ...
    let markers = '';
    if (stealing) {
      if (n === room.round.lockedPosition) return '<div class="timeline-marker locked-marker"><span>LOCKED IN</span><strong>Mystery song</strong><small>Playing team’s pick</small></div>';
      markers = room.round.steals.filter(s => s.position === n).map(steal => {
        const t = room.timelines.find(t => t.id === steal.timelineId);
        return `<div class="timeline-marker ${teamClass(t.team)}"><span>◉ STEAL TOKEN</span><strong>${esc(t.name)}</strong><small>1 token spent</small></div>`;
      }).join('');
    }
    if (!interactive) return markers || (stealing ? '<span class="timeline-gap gap-unavailable" aria-hidden="true">+</span>' : '');
    return `${markers}<button type="button" class="timeline-gap ${state.selected === n ? 'selected' : ''}" data-action="gap" data-position="${n}" aria-label="${stealing ? 'Place steal token' : 'Place song'} ${label}" aria-pressed="${state.selected === n}">${state.selected === n ? `<span>${stealing ? 'YOUR TOKEN' : 'YOUR PICK'}</span>${icon(stealing ? 'crown' : 'music')}<strong>${stealing ? 'Steal this song' : 'New song'}</strong><small>${stealing ? 'Spend one token below' : 'Lock it in below'}</small>` : icon('plus')}</button>`;
  };
  return `<section class="timeline-section"><div class="timeline-heading"><div><h2>${myTurn() ? active.team ? 'Your team’s timeline' : 'Your timeline' : `${esc(active.name)}’s timeline`}<span>${active.cards.length} / ${room.target}</span></h2><p>${stealing ? 'Place steals here. A stolen card goes into the winner’s own timeline in year order.' : placing ? active.team ? 'Choose together. Any teammate can lock in your answer; the first submission counts.' : 'Tap a gap. Let your music memory do the rest.' : room.phase === 'reveal' ? 'A little more music history, settled.' : 'The oldest memories on the left. The newest on the right.'}</p></div><span class="timeline-direction">THEN <span>⟶</span> NOW</span></div><div class="timeline-scroll" id="timeline-scroll"><div class="timeline-track">${gap(0)}${active.cards.map((c, i) => `<article class="song-card card-color-${i % 4}"><span class="card-label">IN THE COLLECTION</span><strong class="card-year">${c.year}</strong><div class="card-divider"></div><h3>${esc(c.title)}</h3><p>${esc(c.artist)}</p><span class="card-corner">${icon('music')}</span></article>${gap(i + 1)}`).join('')}</div></div>${placing ? recognitionForm() : ''}<div class="timeline-bottom"><p>${icon('help')}Same year? The playing team keeps the card.</p>${placing ? `<button id="lock-button" type="submit" form="recognition-form" class="button button-dark" ${state.selected === null || !state.connected || !room.round.startAt || now() < room.round.startAt ? 'disabled' : ''}>Lock it in${icon('check')}</button>` : canSteal() ? `<button id="steal-button" class="button button-dark" data-action="steal" ${state.selected === null || !state.connected ? 'disabled' : ''}>Place token · 1 token${icon('check')}</button>` : `<span class="turn-caption">${stealing ? 'Waiting for steals or passes.' : room.phase === 'reveal' ? 'The stories behind the songs are half the fun.' : `It’s ${esc(active.name)}’s turn.`}</span>`}</div></section>`;
}
function finished() {
  const room = state.room;
  const winners = room.timelines.filter(t => room.winners?.includes(t.id));
  const ranking = [...room.timelines].sort((a, b) => b.cards.length - a.cards.length);
  return `<section class="results"><span class="eyebrow">SIDE B · THE ENCORE</span><div class="winner-icon">${icon('crown')}</div><h1>${winners.some(belongsToMe) ? 'Take a bow.' : 'A round of applause.'}</h1><p class="winner-name">${winners.map(p => esc(p.name)).join(' & ')} ${winners.length > 1 ? 'share the spotlight.' : 'owns the timeline.'}</p><p class="muted">${esc(room.finishReason)}</p><div class="leaderboard">${ranking.map((p, i) => `<div><span class="rank">${String(i + 1).padStart(2, '0')}</span><strong>${esc(p.name)}${belongsToMe(p) ? ' <small>(you)</small>' : ''}</strong><span>${p.cards.length} cards</span></div>`).join('')}</div><button class="button button-dark" data-action="reset" ${!isHost() ? 'disabled' : ''}>${isHost() ? 'One more session?' : 'Waiting for the host'}${icon('replay')}</button><p class="small muted">New shuffle. Fresh start. Same good company.</p></section>`;
}
